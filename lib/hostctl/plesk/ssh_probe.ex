defmodule Hostctl.Plesk.SSHProbe do
  ...
  defp ftp_query_step do
    [
      ...
      "ftp_has_sysuser_id=$(run_plesk db -Ne \"select column_name from information_schema.columns where table_schema = database() and table_name='ftp_users' and column_name='sys_user_id' limit 1\" 2>/dev/null)",
      "if [ -z \"$ftp_login_col\" ] && [ -n \"$ftp_has_sysuser_id\" ]; then",
      "  ftp_rows=$(run_plesk db -Ne \"select s.login, s.home from ftp_users f join sys_users s on f.sys_user_id = s.id order by s.login\" 2>&1)",
      "  if [ $? -ne 0 ]; then",
