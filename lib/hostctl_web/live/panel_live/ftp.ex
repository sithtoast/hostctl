defmodule HostctlWeb.PanelLive.Ftp do
  ...
  def mount(_params, _session, socket) do
    ...
    ftp_dir_options =
      if first_user, do: build_ftp_dir_options(first_user.id), else: []

    {:ok,
     socket
     |> stream(:ftp_accounts, ftp_accounts)
     |> assign(:page_title, "All FTP Accounts")
     |> assign(:active_tab, :panel_ftp)
     |> assign(:users, users)
     |> assign(:selected_user_id, first_user && first_user.id)
  ...
  def handle_event("select_user", %{"user_id" => user_id_str}, socket) do
    ...
    ftp_dir_options = build_ftp_dir_options(user_id)
  ...
  def handle_event("edit_ftp", %{"id" => id}, socket) do
    scope = socket.assigns.current_scope
    account = Enum.find(Hosting.list_all_ftp_accounts(scope), &(to_string(&1.id) == id))

    if account do
      edit_dir_options = build_ftp_dir_options(account.user_id)
      ...
      {:noreply,
       ...
       |> assign(:ftp_dir_options, edit_dir_options)
       |> stream_insert(:ftp_accounts, account)}
    else
      {:noreply, socket}
    end
  ...
  def handle_event("cancel_edit_ftp", _params, socket) do
    # Restore dir_options to the selected_user_id for the create form
    ftp_dir_options =
      if socket.assigns.selected_user_id,
        do: build_ftp_dir_options(socket.assigns.selected_user_id),
        else: []
  ...
  def handle_event("validate_edit_ftp", %{"ftp_account" => params}, socket) do
    ...
    account =
      Enum.find(
        Hosting.list_all_ftp_accounts(scope),
        &(&1.id == socket.assigns.editing_ftp_id)
      )
    ...
    if account do
      ...
      case Hosting.update_ftp_account(account, params) do
        {:ok, updated} ->
          ...
          # Restore dir_options to the create form's selected user
          ftp_dir_options =
            if socket.assigns.selected_user_id,
              do: build_ftp_dir_options(socket.assigns.selected_user_id),
              else: []
  ...
  defp build_ftp_dir_options(user_id) do
    domains = Hosting.list_domains_by_user_id(user_id)
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope} active_tab={@active_tab}>
      <div class="space-y-6">
        <%!-- Header --%>
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">All FTP Accounts</h1>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Global view of all FTP accounts across all users.
          </p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
          ...
          <div class="p-6 space-y-4">
            <%!-- User selector --%>
            <div>
              <label class="block text-sm font-semibold leading-6 text-zinc-800 dark:text-zinc-200 mb-2">
                Account owner
              </label>
              <select
                id="ftp-user-select"
                ...
                name="user_id"
                class="block w-full sm:w-72 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500"
              >
                <%= for user <- @users do %>
